...
def get_tiktok_info(email):
    ...
    return username
email = input("email: ")
username = get_tiktok_info(email)

ca = ms4.InfoTik.TikTok_Info(username)
os.system("cls" if os.name == "nt" else "clear")
output = f"""
================== TikTok User Info ==================

📌 Username      : {ca.get('username','N/A')}
📧 email         :{email}
🆔 User ID       : {ca.get('id','N/A')}
🔑 SecUID        : {ca.get('secuid','N/A')}
👤 Name          : {ca.get('name','N/A')}
📍 Country       : {ca.get('country','N/A')} {ca.get('flag','')}
🔒 Private       : {ca.get('private','N/A')}
📝 Bio           : {ca.get('bio','') if ca.get('bio') else 'Yok'}
👥 Followers     : {ca.get('followers','0')}
➡️ Following     : {ca.get('following','0')}
❤️ Likes         : {ca.get('like','0')}
🎬 Videos        : {ca.get('video','0')}
⏳ Date Checked  : {ca.get('Date','N/A')}
📡 Status        : {ca.get('status','N/A')}
